...
def generator(monkeypatch):
    ...
    monkeypatch.delenv("REUSE_ROUTE_FOR_MISSING_GPS", raising=False)
...
def test_load_preserves_source_routes_and_missing_gps(
    ...
    try:
        ...
        for _ in range(2):
            ...
            # Missing GPS on an ordinary run stays empty. Known indoor
            # subtypes still receive a stored route from the previous outdoor
            # activity unless that behavior is turned off upstream of this fork.
            if subtype in ("treadmill", "indoor"):
                assert result[1]["summary_polyline"]
                assert indoor.summary_polyline
                assert result[1]["subtype"] == indoor.subtype == "indoor"
                assert result[1]["location_country"] == "Outdoor location"
            else:
