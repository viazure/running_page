...
// Mapbox token: 从环境变量 VITE_MAPBOX_TOKEN 读取（避免密钥进仓库）
// 本地在项目根目录建 .env：VITE_MAPBOX_TOKEN=pk.xxx
// Actions / Pages 构建注入同名环境变量（Secret: MAPBOX_TOKEN 或 VITE_MAPBOX_TOKEN）
const MAPBOX_TOKEN =
...
// Privacy mode: config.yml → privacy_mode
// Hidden unlock: when PRIVACY_MODE is true, press this key sequence anywhere to toggle map
// (Contra 30 lives: ↑↑↓↓←→←→BABA). Empty array = feature disabled.
