...
interface StatsCardsProps {
  ...
  /** Tighter mobile/pro layout. Default false = upstream typography/grid. */
  compact?: boolean;
}
...
export function StatsCards({
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          ...
                          style={
                            wd.hasActivity ? { backgroundColor: color } : {}
                          }
                        >
                          {wd.day}
                        </div>
  ...
  return (
    <div className="grid grid-cols-3 gap-2 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,0.95fr)_minmax(0,0.95fr)_minmax(0,1.35fr)] lg:gap-4">
      {/* Yearly Goal */}
      <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-2 transition-all duration-300 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5 hover:shadow-[var(--color-accent)]/5 hover:shadow-lg md:p-4">
        <p className="mb-1 flex items-center gap-1 text-[10px] tracking-wider text-[var(--color-muted)] uppercase md:mb-2 md:gap-1.5 md:text-xs">
          <svg
          ...
          <span className="truncate">{t('yearlyGoal')}</span>
        </p>
        <p className="font-mono text-base font-bold whitespace-nowrap md:text-2xl lg:text-3xl">
          {goal.unit === 'time'
            ? formatHours(yearSeconds)
            : formatDistance(yearDistance)}
          <span className="ml-0.5 text-[10px] font-normal text-[var(--color-muted)] md:ml-1 md:text-sm">
            /{' '}
            {goal.unit === 'time'
              ? `${Math.round(yearGoalMins / 60)}h`
              : `${goal.yearly} km`}
        ...
        <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--color-border)] md:mt-3 md:h-1.5">
          <div
            className="h-full rounded-full bg-[var(--color-accent)] transition-all"
            style={{
              width: `${Math.min(goal.unit === 'time' ? (yearSeconds / (yearGoalMins * 60)) * 100 : (yearDistance / yearGoalMeters) * 100, 100)}%`,
        ...
        <div className="mt-2 hidden items-center justify-between text-sm text-[var(--color-muted)] md:mt-3 md:flex">
          <span className="flex items-center gap-1.5">
            <svg
              className="h-3.5 w-3.5 text-[var(--color-accent)]"
              fill="none"
        ...
        <p
          className={`mt-1 truncate text-[9px] md:mt-1.5 md:text-xs ${yearDiff >= 0 ? 'text-emerald-500' : 'text-red-400'}`}
        >
          {yearDiff >= 0 ? '↗' : '↘'}{' '}
          {goal.unit === 'time'
            ? formatHours(Math.abs(yearDiff))
            : `${formatDistance(Math.abs(yearDiff))} km`}{' '}
          <span className="hidden sm:inline">{t('vsLastYear')}</span>
        </p>
      </div>
      ...
      <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-2 transition-all duration-300 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5 hover:shadow-[var(--color-accent)]/5 hover:shadow-lg md:p-4">
        ...
        <p
          className={`mt-1 truncate text-[9px] md:mt-1.5 md:text-xs ${monthDiff >= 0 ? 'text-emerald-500' : 'text-red-400'}`}
        >
          {monthDiff >= 0 ? '↗' : '↘'}{' '}
          {goal.unit === 'time'
            ? formatHours(Math.abs(monthDiff))
      ...
      <div className="min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-2 transition-all duration-300 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5 hover:shadow-[var(--color-accent)]/5 hover:shadow-lg md:p-4">
        ...
        <div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--color-border)] md:mt-3 md:h-1.5">
          <div
            className="h-full rounded-full bg-[var(--color-accent)] transition-all"
            style={{
              width: `${Math.min(goal.unit === 'time' ? (weekSeconds / (weekGoalMins * 60 || 1)) * 100 : (weekDistance / (weekGoalMeters || 1)) * 100, 100)}%`,
            }}
          />
        </div>
        <div className="mt-2 hidden items-center justify-between text-sm text-[var(--color-muted)] md:mt-3 md:flex">
        ...
        <p
          className={`mt-1 truncate text-[9px] md:mt-1.5 md:text-xs ${weekDiff >= 0 ? 'text-emerald-500' : 'text-red-400'}`}
        >
          {weekDiff >= 0 ? '↗' : '↘'}{' '}
          {goal.unit === 'time'
            ? formatHours(Math.abs(weekDiff))
            : `${formatDistance(Math.abs(weekDiff))} km`}{' '}
          <span className="hidden sm:inline">{t('vsLastWeek')}</span>
        </p>
      </div>

      {/* Streak */}
      <div className="col-span-3 min-w-0 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-2 transition-all duration-300 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5 hover:shadow-[var(--color-accent)]/5 hover:shadow-lg md:p-5 lg:col-span-1">
        <div className="mb-1.5 flex items-center justify-between gap-2 md:mb-2">
          <p className="flex items-center gap-1 text-[10px] tracking-wider text-[var(--color-muted)] uppercase md:gap-1.5 md:text-xs">
            <svg
              className="h-3.5 w-3.5"
              fill="none"
              viewBox="0 0 24 24"
        ...
        <div className="flex items-center gap-2.5 lg:block lg:gap-3">
          <p className="shrink-0 font-mono text-2xl font-bold lg:text-3xl">
            {currentStreak}
          ...
          {(() => {
            ...
            return (
              <div className="flex min-w-0 flex-1 items-center gap-1.5 lg:mt-3">
                <div className="flex shrink-0 flex-col items-center gap-0">
                ...
                {/* Centered cluster (not edge-stretched) so days stay inset from card sides */}
